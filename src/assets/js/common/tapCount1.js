...
let user_id = user_info.user_id,
    uuid = user_info.uuid || _getTjjUuid();

function _getEventId() {
    return new Date().getTime() + mdata.user_id;
}
...
if (!!window.sessionStorage.getItem('mdata')) {
    mdata = JSON.parse(window.sessionStorage.getItem('mdata'))
} else {
    mdata={};
    var muuid=uuid == "wechat"? user_id : uuid;
    mdata.version = getQueryString('version')!=null?getQueryString('version'):-1;
    mdata.uuid = muuid;
    mdata.user_id = user_id || -1;
    window.sessionStorage.setItem('mdata', JSON.stringify(mdata))
}

if (Object.keys(user_info).length > 0) {
    var muuid=uuid == "wechat"? user_id : uuid;
    mdata.user_id=user_id;
    mdata.uuid=muuid?muuid:user_id;
    window.sessionStorage.setItem('mdata', JSON.stringify(mdata))
}
...
var tapConfig = {
    ...
    batchData:
        {
            "app_market": "-1",//应用市场
            "client_version": mdata.version,//客户端版本
    ...
    batchClick: function (type, data, posid) {
        var _this = this
        let now_user_info = _getUserInfo();
        user_id = now_user_info.user_id;
        uuid = now_user_info.uuid || _getTjjUuid();
        // 更新user_id和uuid
        mdata.user_id = now_user_info.user_id;
        mdata.uuid = uuid == "wechat"? user_id : uuid;
        // console.log('mdata.user_id:'+mdata.user_id)
        switch (type) {
            case 1:
                var dataArr = _this.getData();
