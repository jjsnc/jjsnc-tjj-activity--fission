...
<template>
    <div class="main">
            ...
            <header class="head" v-if="showType!=5">
                ...
                <template v-if="showType==4">
                    <div class="center s_other">
                        ...
                        <p class="mt12">累计打卡{{clock_day + surplus_day}}次，订单金额全额返现至现金账户</p>
                    </div>
                </template>
                <div class="punch">
                        ...
                        <div class="new">
                            ...
                            <div>
                                <img
                                    ...
                                    class="punchBt"
                                    @click="clickCardShow=!clickCardShow"
                                >
                            </div>
                        </div>
                    </template>
                    <template v-if="showType==1">
                        ...
                        <card :origin="origin" @handlePunch="handlePunch">
                    ...
                    <template v-if="showType==2">
                        ...
                        <card :origin="origin" @handlePunch="handlePunch"></card>
                    </template>
                    <template v-if="showType==3">
                        <div class="new done_center">
                            <div>
                                    ...
                                    <div class="list-item-right">
                                        ...
                                        <div class="goods-middle">
                                            ...
                                            <div class="goods-orgin-price">
                                                <!-- 打卡<span>{{item.dayNum}}</span>次 全额返现 -->
                                                新用户专享全额返现
                                            </div>
                                        </div>
                    ...
                    <div class="may_like">
                        ...
                        <ul>
                            <li v-for="(item,index) in list" :key="index" @click="toAppGoods(item.goodsId)">
                                <div class="img_box">
                                    <img :src="item.imgUrl||item.goodsCover">
                                </div>
        ...
        <div class="download" @click="download" v-if="showType!=0&&showType!=5&&platForm==0">下载淘集集App，打卡全额返</div>
    ...
    methods: {
        ...
        loadData() {
            if (!this.isLoadData) {
                ...
                this.getGoodsData(this.slideIndex, this.sort_type);
            }
        },
        navChange(obj) {
            this.cate = obj.id;
            ...
            if (
                ...
                // this.page[this.slideIndex] = 1;
                console.log(this.page, this.slideIndex, this.page[this.slideIndex])
                this.getGoodsData(this.slideIndex, obj.sort, obj.isRefresh);
            }
            this.setBoxHeight(this.slideIndex, true);
        },
        handlePunch() {},
        //获取弹幕信息
        getDanmuData() {
            ...
            if (type == 1) {
                ...
                this.$http.get(punchAction, { params }).then(res => {
                    ...
                    if (data && data.result == 1) {
                        ...
                        _this.origin = {
                            isShow: true,
                            days: this.clock_day + 1,
                            dayLength: this.clock_day + this.surplus_day
                        };
            ...
            this.$http.get(punchHome, { params }).then(res => {
                ...
                if (data && data.result == 1) {
                    ...
                    if (data.user_type == 3) {
                        ...
                        let self = this;
                        this.shareData = {
                            title: `坚持打卡${self.surplus_day +
                                self.clock_day}次就可以免费领到这件商品，快来一起打卡吧！`,
                            content: `${
                            ...
                            shareUrl:
                                location.origin +
                                "/clock/view/v1_0_0/index/s_user_id/" +
                                this.user_info.user_id,
                        ...
                        if (this.platForm == 0) {
                            ...
                            new Wechat_api({
                                shareJson: {
                                    title: `坚持打卡${self.surplus_day +
                                        self.clock_day}次就可以免费领到这件商品，快来一起打卡吧！`, // 分享标题
                                    link:
                                        location.origin +
                        ...
                        if (this.platForm == 0) {
                            ...
                            new Wechat_api({
                                shareJson: {
                                    ...
                                    link:
                                        location.origin +
                                        "/clock/view/v1_0_0/index/" // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                                }
                            });
                        }
                        //统计埋点
        ...
        //推荐及排序分类商品
        getGoodsData(index, sort_type, isRefresh) {
            let url = "";
            let params = {};
            if (this.showType == 0) {
                if (this.page[index] < 1 && !isRefresh){
                    this.isLoadData = false;
                    return;
                } 
                if (isRefresh) {
                ...
                if (data && data.result == 1||data && data.status == 1) {
                    if (this.showType == 0) {
                        ...
                        if (isRefresh) {
                            ...
                            _isToTop = this.datas[this.slideIndex].length < 1;
                            this.datas[this.slideIndex].push(...data.data);
                            this.list = this.datas[this.slideIndex];
                            console.log(this.list)
                        }
                        this.page[this.slideIndex] = data.next_page;
                        this.setBoxHeight(this.slideIndex, _isToTop);
                    } else {
                        if(this.page_like==1){
        ...
        // 设置容器高度
        setBoxHeight(index,temp){
            this.$nextTick(() => {
                let actList = this.$refs[`goodsList_${index}`][0];
                console.log(this.$el.querySelector('.nav').offsetTop)
                temp ? window.scrollTo(0,this.$el.querySelector('.nav').offsetTop) : '';
