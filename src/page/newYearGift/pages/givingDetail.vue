...
.luckyDetail{
    ...
    .lucky-content{
        ...
        .lucky1-btn{
            background-image: url(./../assets/images/redpacket_free2@2x.png);
        }
        // 更多商品列表
        .goods-more{
            width: 95.7%;
        ...
        <div v-show="haveData" style="height: 100%;">
            ...
            <div class="lucky-content">
                ...
                <div class="sponsor-info">
                    <img v-lazy="luckyData.shareUserIcon" />
                ...
                <div v-if="luckyData.status == 2" class="lucky-btn" @click="toMain">
                    <span>去看看免费礼物</span>
                </div>
                <div v-if="luckyData.status == 0"  class="lucky-btn lucky1-btn" @click="myShare">
                    <span>邀请好友参与抽奖</span>
                    
                </div>
                <div v-if="luckyData.status == 0 || luckyData.status == 3 || luckyData.status == 4 || (luckyData.status == 1 && !myLucky)"  class="lucky-btn" @click="toMain">
                    <span>我也要免费送礼物</span>
                </div>
                <!-- 领取礼物成功弹窗 -->
                <div v-show="successShow" class="lucky-success-box" @touchmove.prevent>
                    <div class="lucky-success-cont">
                        ...
                        <div class="l-s-btn" @click="toMain"></div>
                    </div>
                </div>
                <!-- 更多礼物精选 -->
                <div class="goods-more" v-if="goodsListData.length > 0">
                    ...
                    <ul>
                        <li v-for="(item, index) in goodsListData" :key="index" @click="toMain">
...
<script>
    ...
    import {toMainFree} from './../assets/js/url.js'
    ...
    export default {
        ...
        created(){
            if(getQueryString('wxLoginBack') == 0){
                this.$msgBox.msgTxt(this.msgTxt = decodeURI(getQueryString('wxLoginMessage')) || '微信授权登录失败，请退出重新进入');
                return;
            }
        ...
        methods: {
                ...
                this.$http.get(luckInfo, {params}).then(res => {
                    ...
                    if(data && data.result == 1){
                        ...
                        if(this.luckyData.status == 1||this.luckyData.status == 3||this.luckyData.status == 4){
                            this.myLucky = getQueryString('user_id') == this.luckyData.userHitArr['userId']
                        }
                ...
                this.$http.get(goodsList,{params}).then(res => {
                    ...
                    if(data && data.result == 1){
                        ...
                        setTimeout(() => {
                            ...
                            // 错误日志上报
                        try {
                            if(errorLogUpload && (this.goodsListData.length<=0)){
                                errorLogUpload({
                                    logLevel: 3,
                                    errType: 21,
                                    apiError: {
            ...
            myShare(){
                ...
                this.$tapConfig.batchClick(0,data_s,0);
                let obj={
                    title:"我送了一份礼物送给大家，快来抢！",
                    content:this.luckyData.goodsName + "\n" + this.luckyData.maxNum + "人参与，必有1人中奖",
                    shareUrl:location.origin + '/newYearGift/view/v1_0_0/luckyDetail/annual_id/' + this.annual_id,
                    imgUrl:this.luckyData.goodsImg,
                    share_blessing:this.luckyData.shareBlessing,
                };
                shareToWx(obj);
            },
            // 领取礼物
