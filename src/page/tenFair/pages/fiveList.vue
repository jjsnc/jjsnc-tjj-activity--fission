            ...
            <div>
                <scrollNav :bigDatas='bigDatas' @navChange="navChange" ref="swiperNavList">
                    ...
                    <div class="goods-box" :ref="`goodsList_${i}`">
                        <div
                            class="good"
                            v-for="(item,i) in typelist.list"
                            :key="i"
                            @click.stop="toGoodsDetail(item.goodsId,item)"
                        >
...
import { shopList,goodsList, danmuList, goodsDetail,orderAdd,shareImg } from "./../assets/js/api.js";
...
export default {
    ...
    methods: {
        ...
        setCartData() {
            ...
            if (Object.keys(userCart).length > 0 && userCart["tenFair"]) {
                let cartAllObj = userCart['tenFair'];
                ...
                if (userCart["tenFair"][this.shop_id]) {
                    this.cartObj = userCart["tenFair"][this.shop_id];

                    if(document.querySelectorAll(".scroll-nav .icon")[this.slideIndex]&&document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].children.length==1&&this.cartObj.num){
                        ...
                        h_node.setAttribute("style","position:absolute;top:-0.5rem;right:0.25rem;width:1rem;height:1rem;line-height:1rem;border-radius:100%;background:red;font-size:0.6875rem;color:#ffffff;letter-spacing:0;text-align:center;")
                        h_node.setAttribute("class","horn")
                        let textnode=document.createTextNode(this.cartObj['num']);
                        h_node.appendChild(textnode);
                        document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].appendChild(h_node);
                    }else if(document.querySelectorAll(".scroll-nav .icon")[this.slideIndex]){
                        ...
                        if(_scount==0){
                            ...
                            if(document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].children[1]){
                                document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].children[1].remove();
                            }
                             if(document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].children[1]){
                                document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].children[1].remove();
                            }
                        }else{
                            document.querySelectorAll(".scroll-nav .icon")[this.slideIndex].children[1].innerHTML=_scount; 
                        }
                    }
                } else {
                    this.cartObj = {};
        ...
        setCartNum() {
            if (this.cartObj.goods_id && this.cartObj.num) {
                ...
                if (_count % 5 == 0) {
                    ...
                    this.footerTip =
                        "已选" +'<span>'+ _count+'</span>' + "件商品，去支付 ¥" +'<span>'+ _count * 2+'</span>';
                } else {
                    this.cartNum =
                        _count + "/" + (Math.floor(_count / 5) + 1) * 5;
                    this.footerTip =
                        "还差" +
                        '<span>'+
                        (5 - (_count % 5))+'</span>' +
                        "件商品，" +'<span>'+
                        (Math.floor(_count / 5) + 1) * 10 +'</span>' +
                        "元" +
        ...
        createOrder() {
            ...
            this.$http.get(orderAdd, { params }).then(res => {
                ...
                if (data.status == 1) {
                    ...
                    this.setCartData();
                    this.cartShow = false;
                    console.log(data.data.orderNo,'data.data.orderNo')
                    userAppPay({
...
.list {
    ...
    .hack-line{
        ...
        &::before{
            ...
            bottom:0 ;
            left: 0;
            background: linear-gradient(180deg, rgba(0,0,0,0.06) 0%, rgba(255,255,255,0) 100%);      
        }
    }
    .goods-box {
