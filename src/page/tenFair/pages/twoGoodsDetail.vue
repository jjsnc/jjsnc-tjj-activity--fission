...
<script >
import { toHomeTwo} from "./../assets/js/url.js";
import {
	getQueryString,
	userAppPay,
	fissile_setCart,
...
export default {
    ...
    methods: {
        ...
        setCartData(){
            ...
            if(Object.keys(userCart).length > 0 && userCart['tenTwo'] && userCart['tenTwo'][this.shopId] ){
                this.cartObj = userCart['tenTwo'][this.shopId];
            }
            this.setCartNum();
            this.setMyGoodsNum();
        },
        ...
        setCartNum(){
            if(this.cartObj.goods_id && this.cartObj.num){
                ...
                if(_count % 2 == 0){
                    ...
                    this.footerTip = '已选'+'<span>'+_count+'件'+'</span>'+'商品，去支付 ¥'+'<span>'+_count*5+'</span>';
                }else{
                    this.cartNum = _count + '/' + (Math.floor(_count / 2) + 1) * 2;
                    this.footerTip = '还差'+'<span>'+(2 - _count % 2)+'件'+'</span>'+'商品，'+'<span>'+(Math.floor(_count / 2) + 1) * 10+'元'+
                    '</span>'+
                ...
                this.cartNum = '0/2';
                this.footerTip= '还差<span>2件</span>商品，<span>10元</span>带回家';
            }
        },
        // 返回店铺操作
        backShop(){
